...
def dataset_loader():
    hotel = hotel_reviews(stem_words=False)
...
def run_pilene_with_all_options():
    lexicon_loaders = (oplexicon, sentilex, reli_lexicon)
    dataset_loaders = (hotel_reviews, reli_reviews)
    stem_words_values = apply_mask_values = (True, False)
    mask_max_steps_values = range(10, 16)

    all_results = []
    results_measures = []
    index = 0

    for lex_loader, dataset_loader, stem_words, mask_max_steps, apply_mask \
            in itertools.product(lexicon_loaders,
                                 dataset_loaders,
                                 stem_words_values,
        ...
        if apply_mask is False and mask_max_steps != 10:
            continue

        result, measure = socal_result(stem_words, mask_max_steps, apply_mask, lex_loader, dataset_loader)
    ...
    params_header = u'{:16} {:16} {:4} {:10} {:8} {:6} {:6} {:6} {:6}'.format(
        'lexicon', 'dataset', 'stem', 'mask_steps', 'use_mask', 'acc', 'mcc', 'fsc', 'wfsc'
    )
...
def socal_result(index, stem_words, mask_max_steps, apply_mask, lex_loader, dataset_loader):
    ...
    result = u'\n'.join([u'-' * 100, u'{:16} {:16} {:^4} {:^10} {:^8} {:2.2f}   {:2.2f}   {:2.2f}   {:2.2f}'.format(
            lex_loader.__name__, dataset_loader.__name__, stem_words, mask_max_steps,
